...
for j,y in enumerate(myPicList):
    ...
    for x,r in enumerate(roi):
        ...
        imgCropped = imgScan[r[0][1]:r[1][1],r[0][0]:r[1][0]]
        #cv2.imshow(str(x),imgCropped)

        if r[2] == 'text':
            myData.append(pytesseract.image_to_string(imgCropped))
            print(f'{r[3]}: {pytesseract.image_to_string(imgCropped)}')

        if r[2]=='box':
            imgGray=cv2.cvtColor(imgCropped,cv2.COLOR_BGR2GRAY)
            imgThres=cv2.threshold(imgGray,170,255,cv2.THRESH_BINARY_INV)[1]
            totalPixels=cv2.countNonZero(imgThres)
            if totalPixels>500:
                totalPixels=1
            else:
                totalPixels=0
        ...
        cv2.putText(imgShow,str(myData[x]),(r[0][0],r[0][1]),cv2.FONT_HERSHEY_SIMPLEX,2.5,(0,0,255),4)
